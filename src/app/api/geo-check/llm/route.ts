...
export async function POST(req: NextRequest) {
  try {
    ...
    let questionSource: "generated" | "curated" | "descriptor" | "domain_inferred" = "descriptor";

    // V1 priority: generated questions from /quick → curated → descriptor
    // For crawl_failed: questions were already generated by /quick from domain inference
    if (report.generated_questions && Array.isArray(report.generated_questions) && report.generated_questions.length >= 6) {
      prompts = report.generated_questions.slice(0, 6);
      questionSource = (report.question_source as any) || "generated";
      verticalResolved = report.crawl_failed
        ? `Domain inference (${reportVertical})`
        : `Generated (${reportVertical})`;
      console.log(`[GEO-Check] Using ${prompts.length} generated questions (source: ${questionSource})`);
