        ...
        <div className="flex-1 min-w-0 max-w-[60%] flex items-center gap-1.5 pl-2">
          ...
          {isEditingTitle ? (
            <input
              ...
              className="nodrag nopan w-full bg-transparent border-none outline-none text-xs font-semibold tracking-wide text-[var(--c-text-secondary)] placeholder:text-[var(--c-text-muted)] uppercase"
            />
          ) : (
            <span
              className="nodrag text-xs font-semibold uppercase tracking-wide text-[var(--c-text-tertiary)] cursor-text truncate"
        ...
        <div className={`shrink-0 flex items-center gap-1 pr-1 transition-opacity duration-200 -translate-y-1 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="relative shrink-0 flex items-center gap-1" ref={commentPopoverRef}>
            <button
              ...
              className={`nodrag nopan p-0.5 rounded transition-colors ${
                comment
                  ...
                  : "text-[var(--c-text-muted)] hover:text-[var(--c-text)] border border-[var(--c-border)]"
              }`}
            ...
            {(showCommentTooltip || isCommentFocused) && comment && !isEditingComment && tooltipPosition && createPortal(
              <div
                ref={tooltipRef}
                className="fixed z-[9999] p-3 text-sm text-[var(--c-text)] bg-[var(--c-modal-bg)] border border-[var(--c-modal-border)] rounded-lg shadow-xl"
                style={{
              ...
              >
                {isCommentFocused && commentNavigation && (
                  <div className="flex items-center justify-center gap-3 mb-2 pb-2 border-b border-[var(--c-border-subtle)]">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        commentNavigation.onPrevious();
                      }}
                      className="nodrag nopan w-6 h-6 flex items-center justify-center text-[var(--c-text-tertiary)] hover:text-[var(--c-text)] hover:bg-[var(--c-bg-hover)] rounded transition-colors"
                    ...
                    <span className="text-xs text-[var(--c-text-tertiary)] min-w-[32px] text-center">
            ...
            {isEditingComment && (
              <div className="absolute z-[60] right-0 top-full mt-1 w-64 p-2 bg-[var(--c-node-bg)] border border-[var(--c-border)] rounded shadow-lg">
                <textarea
                  ...
                  autoFocus
                  className="nodrag nopan nowheel w-full h-20 p-2 text-xs text-[var(--c-text)] bg-[var(--c-input-bg)] border border-[var(--c-input-border)] rounded resize-none focus:outline-none focus:ring-1 focus:ring-[var(--c-input-focus)]"
                />
                <div className="flex justify-end gap-2 mt-2">
                  <button
                    ...
                    className="px-2 py-1 text-xs text-[var(--c-text-tertiary)] hover:text-[var(--c-text)] transition-colors"
                  >
                    Cancel
                  </button>
                  <button
          ...
          {canExpand && onExpandNode && (
            <div className="relative shrink-0 group">
              <button
                ...
                className="nodrag nopan p-0.5 rounded transition-all duration-200 ease-in-out text-[var(--c-text-muted)] group-hover:text-[var(--c-text)] border border-[var(--c-border)] flex items-center overflow-hidden group-hover:pr-2"
                title="Expand editor"
              >
                <svg
          ...
          {canRun && onRunNode && (
            <div className="relative shrink-0 group">
              <button
                ...
                className="nodrag nopan p-0.5 rounded transition-all duration-200 ease-in-out text-[var(--c-text-muted)] group-hover:text-[var(--c-text)] border border-[var(--c-border)] flex items-center overflow-hidden group-hover:pr-2 disabled:opacity-50 disabled:cursor-not-allowed"
                title="Run this node"
              >
