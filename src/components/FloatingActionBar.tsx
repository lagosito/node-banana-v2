...
function NodeButton({ type, label }: NodeButtonProps) {
  ...
  return (
    <button
      ...
      className="px-2.5 py-1.5 text-[11px] font-medium text-[var(--c-text-tertiary)] hover:text-[var(--c-text)] hover:bg-[var(--c-bg-hover)] rounded transition-colors cursor-grab active:cursor-grabbing"
    >
      {label}
    </button>
  );
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className="px-2.5 py-1.5 text-[11px] font-medium text-[var(--c-text-tertiary)] hover:text-[var(--c-text)] hover:bg-[var(--c-bg-hover)] rounded transition-colors flex items-center gap-1"
      >
        Generate
        <svg
      ...
      {isOpen && (
        <div className="absolute bottom-full left-0 mb-2 bg-[var(--c-fab-bg)] border border-[var(--c-border-subtle)] rounded-lg shadow-xl overflow-hidden min-w-[140px]">
          <button
            onClick={() => handleAddNode("nanoBanana")}
            draggable
            onDragStart={(e) => handleDragStart(e, "nanoBanana")}
            className="w-full px-3 py-2 text-left text-[11px] font-medium text-[var(--c-text-secondary)] hover:bg-[var(--c-bg-hover)] hover:text-[var(--c-text)] transition-colors flex items-center gap-2 cursor-grab active:cursor-grabbing"
          >
...
function AllNodesMenu() {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute bottom-full left-0 mb-2 bg-[var(--c-fab-bg)] border border-[var(--c-border)] rounded-lg shadow-xl overflow-hidden min-w-[180px] max-h-[400px] overflow-y-auto">
          {ALL_NODES_CATEGORIES.map((category, catIndex) => (
            <div key={category.label}>
              <div className={`px-3 py-1 text-[10px] text-[var(--c-text-muted)] uppercase tracking-wide${catIndex > 0 ? " border-t border-[var(--c-border-subtle)]" : ""}`}>
                {category.label}
              </div>
...
export function FloatingActionBar() {
  ...
  return (
    <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50">
      <div className="flex items-center gap-0.5 bg-[var(--c-fab-bg)]/95 rounded-lg shadow-lg border border-[var(--c-border)]/80 px-1.5 py-1">
        ...
        {/* All models button */}
        <div className="w-px h-5 bg-[var(--c-border)] mx-1.5" />
        <button
          onClick={() => setModelSearchOpen(true)}
          title="Browse models"
          className="px-2.5 py-1.5 text-[11px] font-medium text-[var(--c-text-tertiary)] hover:text-[var(--c-text)] hover:bg-[var(--c-bg-hover)] rounded transition-colors"
        >
          All models
        </button>
        ...
        <button
          ...
          className="p-1.5 text-[var(--c-text-tertiary)] hover:text-[var(--c-text)] hover:bg-[var(--c-bg-hover)] rounded transition-colors"
        >
          {edgeStyle === "angular" ? (
        ...
        <div className="relative flex items-center" ref={runMenuRef}>
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-medium transition-colors ${
              isRunning
                ? "bg-[var(--c-text)] text-[var(--c-bg)] hover:bg-[var(--c-text-secondary)] rounded"
                : valid
                ? "bg-[var(--c-text)] text-[var(--c-bg)] hover:bg-[var(--c-text-secondary)] rounded-l"
                : "bg-[var(--c-bg-elevated)] text-[var(--c-text-muted)] cursor-not-allowed rounded"
            }`}
          >
            {isRunning ? (
              <>
          ...
          {!isRunning && valid && (
            <button
              ...
              className="flex items-center self-stretch px-1.5 rounded-r bg-[var(--c-text)] text-[var(--c-bg)] hover:bg-[var(--c-text-secondary)] border-l border-[var(--c-text-muted)]/30 transition-colors"
              title="Run options"
            >
              <svg
          ...
          {runMenuOpen && !isRunning && (
            <div className="absolute bottom-full right-0 mb-2 bg-[var(--c-fab-bg)] border border-[var(--c-border-subtle)] rounded-lg shadow-xl overflow-hidden min-w-[180px]">
              <button
                onClick={() => {
                  executeWorkflow();
                  setRunMenuOpen(false);
                }}
                className="w-full px-3 py-2 text-left text-[11px] font-medium text-[var(--c-text-secondary)] hover:bg-[var(--c-bg-hover)] hover:text-[var(--c-text)] transition-colors flex items-center gap-2"
              >
              ...
              <button
                ...
                className={`w-full px-3 py-2 text-left text-[11px] font-medium transition-colors flex items-center gap-2 ${
                  selectedNode
                    ? "text-[var(--c-text-secondary)] hover:bg-[var(--c-bg-hover)] hover:text-[var(--c-text)]"
                    : "text-[var(--c-text-muted)] cursor-not-allowed"
                }`}
